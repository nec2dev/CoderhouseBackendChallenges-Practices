...
const pathFile = `Class008/challengeDeliverable004/model/productos.json`;
...
class Contenedor {
    ...
    try {
      ...
      if (!datos) {
        try {
          ...
          console.log(
            `Se produjo un error al intentar inicializar el archivo: ${error}`
          );
        }
      }
      return true;
  ...
  async getById(id) {
    if (!isNaN(id)) {
      try {
        ...
        console.log(
          `Error al leer el archivo en el método getById(): ${error}`
        );
      }
    } else {
  ...
  async save(producto) {
    ...
    try {
      ...
      if (!isNaN(maxIndice)) {
        ...
        console.log(
          `No se pudo calcular el último id en el archivo, id: ${maxIndice}`
        );
      }
      return maxIndice;
    } catch (error) {
  ...
  async updateById(id, product) {
    try {
      ...
      console.log(
        `Se produjo un error al intentar actualizar el archivo: ${error}`
      );
    }
  }
